...
The fetch script needs the `gh` CLI authenticated; it targets `a10kiloham/Anthias` by default. Pass `--ref <branch>` to pull from a different branch's most recent run, or `--repo <owner>/<repo>` to point at a different repo. Without screenshots, Hugo still builds — the slider section simply omits.
